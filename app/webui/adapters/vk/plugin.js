...
async function mountInstanceConfig(ctx) {
  const { container, definition, instance } = ctx;
  ...
  wrapper.innerHTML = ctx.template || '';
  const form = wrapper.querySelector('form') || document.createElement('form');
  if (!form.parentElement) {
    form.className = 'instance-form adapter-vk__form';
    wrapper.appendChild(form);
  }
  form.dataset.adapterKey = 'vk';
  form.dataset.instanceId = instance.id;
  form.querySelector('[data-slot="display-name"]')?.replaceChildren(document.createTextNode(instance.display_name || 'VK'));
  ...
  if (extraSlot) {
    extraSlot.innerHTML = '<div class="vk-oauth-section"></div>';
  }
...
async function mountRouteConfig(ctx) {
  ...
  div.className = 'adapter-vk__route-config';
  div.innerHTML = template || '<div class="adapter-vk__route-template"></div>';
  const root = div.firstElementChild || div;
  root.innerHTML = `
    <div class="muted">No additional route settings for VK</div>
    ...
    <div class="help">adapter: <span class="mono">${escapeHtml(instance?.adapter_key || 'vk')}</span></div>
    <div class="help">
