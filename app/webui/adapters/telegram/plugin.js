...
async function mountInstanceConfig(ctx) {
  ...
  if (!form.parentElement) {
    form.className = 'instance-form adapter-telegram__form';
    wrapper.appendChild(form);
  }
  form.dataset.adapterKey = 'telegram';
  form.dataset.instanceId = instance.id;
  form.querySelector('[data-slot="display-name"]')?.replaceChildren(document.createTextNode(instance.display_name || 'Telegram'));
  ...
  if (extraSlot) extraSlot.innerHTML = '';
...
async function mountRouteConfig(ctx) {
  ...
  div.className = 'adapter-telegram__route-config';
  div.innerHTML = template || '<div class="adapter-telegram__route-template"></div>';
  const root = div.firstElementChild || div;
  root.innerHTML = `
    <div class="muted">No additional route settings for Telegram</div>
    ...
    <div class="help">adapter: <span class="mono">${escapeHtml(instance?.adapter_key || 'telegram')}</span></div>
    <div class="help">
