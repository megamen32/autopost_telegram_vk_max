...
MAX_DEFINITION = AdapterDefinition(
    ...
    fields=[
        ...
        AdapterSettingField("receive_mode", "Режим получения событий", "choice", "simple", False, False, "Long polling удобен по умолчанию. Webhook нужен, если бот работает за публичным HTTPS endpoint.", options=[{"value": "long_poll", "label": "Long polling"}, {"value": "webhook", "label": "Webhook"}], default="long_poll", visible_when=when_true("receive_updates")),
        ...
        AdapterSettingField("webhook_url", "Webhook URL", "str", "advanced", False, False, "Нужен только если выбран webhook-режим. Должен быть публичный HTTPS URL этого инстанса.", visible_when=when_all(when_true("receive_updates"), when_eq("receive_mode", "webhook"))),
        AdapterSettingField("secret", "Webhook secret", "str", "advanced", False, True, "Секрет для проверки заголовка X-Max-Bot-Api-Secret. Используется только в webhook-режиме.", visible_when=when_all(when_true("receive_updates"), when_eq("receive_mode", "webhook"))),
        AdapterSettingField("update_types", "Типы обновлений", "list_str", "advanced", False, False, "Какие типы событий получать. Обычно достаточно message_created.", visible_when=when_true("receive_updates")),
